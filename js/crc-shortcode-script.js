...
var crc_give_currency_html = document.getElementById("get_currency");
if (crc_give_currency_html) {
  var crc_give_currency = document.getElementById("get_currency").innerHTML;
}
...
const crc_link_btn = document.getElementById("crc_link_btn");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (sidebar_right) {
      ...
      const isClickOnToggleButton_right = toggleButton_right.contains(
        event.target
      );
...
function crc_calculateGetVal() {
  ...
  if (crc_currencyIdLeft.value != "" && crc_currencyIdRight.value != "") {
    // Get the conversion, fee, and input values
    ...
    const crc_fee_percentage = parseFloat(crc_feeInput.value); // Fee as a percentage
    const crc_giveValue = parseFloat(crc_giveValInput.value);

    if (crc_giveValue >= crc_minimum_amount) {
      // Calculate the fee as a percentage of the given value
      const crc_fee = (crc_fee_percentage / 100) * crc_giveValue;

      // Perform the calculation
      const crc_getValue = (crc_giveValue - crc_fee) * crc_conversion;

      // Update the getValInput value with the calculated result
      crc_getValInput.value = crc_getValue.toFixed(2); // Round to 2 decimal places
      next_button = document.getElementById("next_button");

      // Enable the "Next" button
      next_button.classList.remove("crc_disabled");
    } else {
      // Disable the "Next" button if the give value is below the minimum
      next_button.classList.add("crc_disabled");
    }
  } else {
    // Disable the "Next" button if currencies are not selected
    next_button.classList.add("crc_disabled");
  }
}
...
function toggleSidebar_left() {
  ...
  var sidebar_left_content = document.getElementById("q-item-left");
  var cross_icons = document.getElementById("crc-close-btn-left");
  cross_icons.style.display =
    cross_icons.style.display === "grid" ? "none" : "grid";
  sidebar_left.style.width =
    sidebar_left.style.width === "300px" ? "0" : "300px";
  sidebar_left_content.style.display =
    sidebar_left_content.style.display === "flex" ? "none" : "flex";
}
...
function toggleSidebar_right() {
  ...
  cross_icons.style.display =
    cross_icons.style.display === "grid" ? "none" : "grid";
  sidebar_right.style.width =
    sidebar_right.style.width === "300px" ? "0" : "300px";
  sidebar_right_content.style.display =
    sidebar_right_content.style.display === "flex" ? "none" : "flex";
}

function openPopup() {
    // Parse input values
    var crc_conversion = parseFloat(crc_conversionInput.value);
    var crc_fee_percentage = parseFloat(crc_feeInput.value); // Fee as a percentage
    var crc_giveValue = parseFloat(crc_giveValInput.value);
  
    // Initialize variables for calculations
    var crc_fee = 0;
    var crc_totalValue = 0;
    var crc_getValue = 0;
    ...
    if (crc_currencyIdLeft.value != "" && crc_currencyIdRight.value != "") {
      // Calculate the fee as a percentage of the given value
      crc_fee = (crc_fee_percentage / 100) * crc_giveValue;
  
      // Perform the calculations
      crc_totalValue = crc_giveValue * crc_conversion; // Total value before fee deduction
      crc_getValue = (crc_giveValue - crc_fee) * crc_conversion; // Final amount received
    } else {
      // Reset values if currencies are not selected
      crc_conversion = 0;
      crc_fee = 0;
      crc_getValue = 0;
      crc_totalValue = 0;
    ...
    // Display the popup and update the values
    ...
    crc_platform_fee.innerHTML = crc_fee_percentage;
...
function handleCurrencyOptionClick(
  currency_symbol,
  inputId,
  id,
  currencyId,
  ...
  if (side == "left") {
    ...
    document.getElementById("currency_symbol_right").innerHTML =
      currency_symbol;
    crc_get_rates();
  }
  ...
  if (side == "left") {
    document.getElementById("vtov").innerHTML =
      '<span data-v-4e76d53a="" id="give_currency" class="name">' +
      crc_give_currency +
      "</span>";
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      if (response.success) {
        ...
        var currencyOptions = document.querySelectorAll(
          '.currency-option[data-side="right"]'
        );
...
function crc_get_rates() {
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      if (response.success) {
        document.getElementById("crc_rates").innerHTML =
          response.data.conversion_rate;
        document.getElementById("crc_fees").innerHTML =
          response.data.platform_fee;
        document.getElementById("vmin").innerHTML =
          response.data.minimum_amount;
        document.getElementById("crc_conversion").value =
          response.data.conversion_rate;
        ...
        document.getElementById("currency_redirect_link").value =
          response.data.currency_redirect_link;
...
function swapCurrencies() {
  ...
  const left_currency_symbol = document.getElementById(
    "currency_symbol_left"
  ).innerHTML;
  const right_currency_symbol = document.getElementById(
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      if (response.success) {
        ...
        document.getElementById("currency_redirect_link").value =
          response.data.currency_redirect_link;
        document.getElementById("currency_symbol_left").innerHTML =
          right_currency_symbol;
        document.getElementById("currency_symbol_right").innerHTML =
          left_currency_symbol;
        crc_link_btn.href = response.data.currency_redirect_link;
...
function showToastMessage(message) {
  ...
  toast.style.cssText =
    "position: fixed; bottom: 50px; left: 50%; transform: translateX(-50%); background-color: rgba(0, 0, 0, 0.7); color: #fff; padding: 10px 20px; border-radius: 5px; z-index: 9999;";
