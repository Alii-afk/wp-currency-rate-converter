 ...
 * Plugin Uri: https://alihaiderhamadani.com/wp-currency-rate-converter
...
register_uninstall_hook(__FILE__, 'crc_delete_currency_table');
...
function crc_delete_currency_table()
{
    global $wpdb;
...
function crc_add_currency()
{
    ...
    if ($result) {
        ...
        // Fetch all currencies after addition
        $rates = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}crc_currency_listing ORDER BY currency_created_at DESC");
        if ($rates) {
            foreach ($rates as $rate) {
                $table .= "<tr>";
                $table .= "<td><img style='width: 30px;' src='{$rate->currency_logo}'></td>";
                $table .= "<td>{$rate->currency_symbol}</td>";
                $table .= "<td>{$rate->currency_name}</td>";
                $table .= "<td>
...
function crc_switch_currency_conversions()
{
    ...
    if (!empty($result)) {
        ...
        $response = array(
            ...
            'currency_redirect_link' => $result[0]->currency_redirect_link 
        );
